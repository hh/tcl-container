...
--enable-build-info="static build for ii" | tee config-static.out

make -j $MAKE_JOBS | tee build-static.out
cd src
...
mkdir -p /tmp/stage/squid/usr/local/sbin
cp /output/squid /tmp/stage/squid/usr/local/sbin
